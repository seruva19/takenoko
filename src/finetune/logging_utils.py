...
class NetworkLoggingUtils:
    ...
    def log_detailed_network_info(transformer: Any, args: argparse.Namespace) -> None:
        ...
        for name, param in transformer.named_parameters():
            ...
            if param.requires_grad:
                ...
                param_type = NetworkLoggingUtils._categorize_parameter(name, param_categories, param_count)
    ...
    @staticmethod
    def _categorize_parameter(name: str, param_categories: Dict[str, int], param_count: int) -> str:
        ...
        if any(
            ...
            x in name_lower for x in ["modulation", "ada", "scale", "shift"]
        ):
    ...
    def log_training_configuration(args: argparse.Namespace, trainer_instance) -> None:
        ...
        if hasattr(trainer_instance, 'use_stochastic_rounding'):
            logger.info(f"  Stochastic rounding: {trainer_instance.use_stochastic_rounding}")
    ...
    def log_checkpoint_debug_info(restored_step, starting_epoch, global_step, args):
        ...
        if getattr(args, 'verbose_logging', False) or getattr(args, 'debug', False):
            ...
            logger.info(f"   - max_train_steps: {getattr(args, 'max_train_steps', 'Not set')}")
            logger.info(f"   - max_train_epochs: {getattr(args, 'max_train_epochs', 'Not set')}")
            if hasattr(args, 'max_train_steps') and args.max_train_steps:
                logger.info(f"   - Will global_step >= max_train_steps? {global_step >= args.max_train_steps}")
    ...
    def log_epoch_range_info(starting_epoch, max_epochs):
        ...
        if starting_epoch > 0:
            logger.info(f"🔄 Resuming training: epoch {starting_epoch+1}/{max_epochs} ({epochs_to_train} epochs remaining)")
        else:
    ...
    @staticmethod
    def log_parameter_validation_info(dit_params_count, param_names, args, text_encoder=None):
...
class TrainingProgressLogger:
    ...
    def log_step_info(global_step, loss_item, learning_rate, logger_obj=logger):
        ...
        logger_obj.info(f"Step {global_step}: Loss = {loss_item:.6f}, LR = {learning_rate}")
    
    @staticmethod
    def log_sampling_debug(global_step, should_sample, sampling_manager_exists, vae_exists):
