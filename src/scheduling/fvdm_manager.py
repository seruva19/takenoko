...
class FVDMManager:
    ...
    def _initialize_components(self):
        ...
        self.eval_temporal_metrics = getattr(self.args, 'fvdm_eval_temporal_metrics', False)
        self.eval_frequency = getattr(self.args, 'fvdm_eval_frequency', 1000)
        self.temporal_loss_weight = getattr(self.args, 'fvdm_temporal_consistency_weight', 0.0)
        self.diversity_loss_weight = getattr(self.args, 'fvdm_frame_diversity_weight', 0.0)
        self.integrate_adaptive = getattr(self.args, 'fvdm_integrate_adaptive_timesteps', False)
    ...
    def _log_initialization(self):
        ...
        logger.info(f"   PTSS Mode: {'Adaptive' if getattr(self.args, 'fvdm_adaptive_ptss', False) else 'Fixed'}")
        logger.info(f"   PTSS Probability: {getattr(self.args, 'fvdm_ptss_p', 0.2)}")
        logger.info(f"   Adaptive Integration: {self.integrate_adaptive}")
        logger.info(f"   Temporal Metrics: {self.eval_temporal_metrics}")
        logger.info(f"   Additional Loss Components: {self.temporal_loss_weight > 0 or self.diversity_loss_weight > 0}")
    ...
    def get_noisy_input_and_timesteps(
        ...
        if not self.enabled:
            ...
            logger.warning("FVDM Manager called when disabled - this indicates a code issue")
            return latents, torch.zeros(latents.shape[0], device=self.device), torch.zeros_like(latents)
    ...
    def record_training_step(
        ...
        try:
            ...
            is_async = timestep_variance.item() > 0.01  # Threshold for considering it async
    ...
    def get_additional_loss(
        ...
        """
        if not self.enabled or (self.temporal_loss_weight == 0 and self.diversity_loss_weight == 0):
    ...
    def should_log_metrics(self, step: int) -> bool:
        ...
        return (self.enabled and 
                self.eval_temporal_metrics and 
                step > 0 and 
                step % self.eval_frequency == 0)
