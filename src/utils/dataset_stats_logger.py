...
def log_dataset_stats_to_tensorboard(
    ...
    try:
        ...
        _log_dataset_group_stats(
            tb_writer, train_dataset_group, prefix="dataset/train", step=0
        )

        # Log validation dataset stats if available
        if val_dataset_group is not None:
            _log_dataset_group_stats(
                tb_writer, val_dataset_group, prefix="dataset/val", step=0
            )
        ...
        log_bucket_distribution_to_tensorboard(
            accelerator, train_dataset_group, prefix="dataset/train/buckets", step=0
        )

        # Log bucket distribution for validation data if available
        if val_dataset_group is not None:
            log_bucket_distribution_to_tensorboard(
                accelerator, val_dataset_group, prefix="dataset/val/buckets", step=0
            )
...
def _log_dataset_group_stats(
    ...
    if all_batch_sizes:
        tb_writer.add_scalar(
            f"{prefix}/batch_size_mean", np.mean(all_batch_sizes), step
        )
    ...
    if len(all_batch_sizes) > 0:
        tb_writer.add_histogram(
            f"{prefix}/distribution/batch_sizes", np.array(all_batch_sizes), step
        )

    if len(all_repeats) > 0:
        tb_writer.add_histogram(
            f"{prefix}/distribution/num_repeats", np.array(all_repeats), step
        )

    if len(item_counts) > 0:
        tb_writer.add_histogram(
            f"{prefix}/distribution/items_per_dataset", np.array(item_counts), step
        )

    if len(resolution_areas) > 0:
        tb_writer.add_histogram(
            f"{prefix}/distribution/resolution_areas", np.array(resolution_areas), step
        )

    if len(aspect_ratios) > 0:
        tb_writer.add_histogram(
            f"{prefix}/distribution/aspect_ratios", np.array(aspect_ratios), step
        )
    ...
    if total_segments > 0:
        ...
        if effective_items > 0:
            tb_writer.add_scalar(
                f"{prefix}/effective_items_per_epoch", effective_items, step
            )
...
def log_temporal_stats_to_tensorboard(
    ...
    try:
        ...
        if "median_frames" in stats:
            tb_writer.add_scalar(
                f"{prefix}/median_frames", stats["median_frames"], step
            )
        ...
        if len(frame_counts) > 0:
            tb_writer.add_histogram(
                f"{prefix}/distribution/frame_counts", np.array(frame_counts), step
            )
        ...
        if "videos_analyzed" in temporal_stats_dict:
            ...
            if total_videos > 0:
                short_pct = (
                    temporal_stats_dict.get("short_videos", 0) / total_videos * 100
                )
                perfect_pct = (
                    temporal_stats_dict.get("perfect_fit", 0) / total_videos * 100
                )
                long_pct = (
                    temporal_stats_dict.get("long_videos", 0) / total_videos * 100
                )
        ...
        if "total_segments" in temporal_stats_dict:
            tb_writer.add_scalar(
                f"{prefix}/total_segments", temporal_stats_dict["total_segments"], step
            )
        if "avg_segments_per_video" in temporal_stats_dict:
            tb_writer.add_scalar(
                f"{prefix}/avg_segments_per_video",
                temporal_stats_dict["avg_segments_per_video"],
                step,
            )
...
def log_bucket_distribution_to_tensorboard(
    ...
    try:
        ...
        if len(bucket_areas) > 0:
            tb_writer.add_histogram(
                f"{prefix}/distribution/bucket_areas", np.array(bucket_areas), step
            )
        ...
        if len(bucket_item_counts) > 0:
            ...
            tb_writer.add_scalar(
                f"{prefix}/total_buckets", len(bucket_item_counts), step
            )
            tb_writer.add_scalar(
                f"{prefix}/mean_items_per_bucket", np.mean(bucket_item_counts), step
            )
            tb_writer.add_scalar(
                f"{prefix}/max_items_per_bucket", np.max(bucket_item_counts), step
            )
            tb_writer.add_scalar(
                f"{prefix}/min_items_per_bucket", np.min(bucket_item_counts), step
            )
            tb_writer.add_scalar(
                f"{prefix}/std_items_per_bucket", np.std(bucket_item_counts), step
            )
