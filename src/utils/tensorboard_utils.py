...
def get_default_metric_descriptions() -> Dict[str, str]:
    ...
    return {
        ...
        "train/effective_batch_size": "Effective synchronized batch size after distributed batch summation and gradient accumulation (informational).",
        "train/loss_std_in_batch": "Population standard deviation of per-sample batch loss after distributed gathering; highlights spread between easy and hard items (lower is better).",
        "train/loss_var_in_batch": "Population variance of per-sample batch loss after distributed gathering (lower is better).",
        "train/loss_hard_topk_mean": "Mean loss of the hardest configured top-k fraction of samples in the gathered batch (lower is better).",
        "train/loss_max_in_batch": "Maximum per-sample loss observed in the gathered batch; useful for outlier detection (lower is better).",
        ...
        "train/optical_flow_error": "Alias of the optical-flow reconstruction error term, surfaced explicitly for diagnostics when optical-flow loss is active (lower is better).",
        ...
        "video/loss_first_frame": "Mean boundary-frame loss on the first latent frame for video batches; useful for first-frame conditioning drift checks (lower is better).",
        "video/loss_last_frame": "Mean boundary-frame loss on the last latent frame for video batches; helps spot tail-frame degradation (lower is better).",
        "video/loss_delta": "Last-frame loss minus first-frame loss on video batches; positive values indicate later frames are worse (closer to 0 is better).",
