...
class FVDMTrainingMetrics:
    ...
    def reset(self):
        ...
        self.recent_async_flags: deque[bool] = deque(maxlen=1000)

    def compute_temporal_consistency_loss(
        self, frames: torch.Tensor, reduction: str = "mean"
    ) -> torch.Tensor:
        """
        ...
        if frames.size(2) >= 3:
            ...
            if reduction == "none":
                consistency_loss = F.smooth_l1_loss(
                    frame_diff, torch.zeros_like(frame_diff), reduction="none", beta=0.1
                )
            else:
                consistency_loss = F.smooth_l1_loss(
                    frame_diff,
    ...
    def compute_frame_diversity_score(
        self, frames: torch.Tensor, sample_pairs: int = 10
    ) -> torch.Tensor:
        """
        ...
        """
        B, C, num_frames, H, W = frames.shape

        if num_frames < 2:
            return torch.tensor(0.0, device=frames.device)

        # Flatten spatial dimensions for easier computation
        frames_flat = frames.view(B, C * H * W, num_frames)

        # Sample frame pairs to avoid O(F^2) computation
        max_pairs = num_frames * (num_frames - 1) // 2
        ...
        for i in range(num_frames):
            for j in range(i + 1, num_frames):
                ...
                dist = F.mse_loss(
                    frames_flat[:, :, i], frames_flat[:, :, j], reduction="mean"
                )
    ...
    def compute_timestep_variance(self, timesteps: torch.Tensor) -> torch.Tensor:
        """
    ...
    def update_sampling_stats(
        self, is_async: bool, ptss_probability: Optional[float] = None
    ):
        """
        ...
        self.recent_async_flags.append(bool(is_async))
    ...
    def get_stats(self) -> Dict[str, float]:
        ...
        if self.temporal_consistency_losses:
            stats["fvdm/avg_temporal_consistency"] = sum(
                self.temporal_consistency_losses
            ) / len(self.temporal_consistency_losses)
            stats["fvdm/recent_temporal_consistency"] = sum(
                self.temporal_consistency_losses[-10:]
            ) / min(10, len(self.temporal_consistency_losses))

        if self.frame_diversity_scores:
            stats["fvdm/avg_frame_diversity"] = sum(self.frame_diversity_scores) / len(
                self.frame_diversity_scores
            )
            stats["fvdm/recent_frame_diversity"] = sum(
                self.frame_diversity_scores[-10:]
            ) / min(10, len(self.frame_diversity_scores))

        if self.timestep_variance_scores:
            stats["fvdm/avg_timestep_variance"] = sum(
                self.timestep_variance_scores
            ) / len(self.timestep_variance_scores)
            stats["fvdm/recent_timestep_variance"] = sum(
                self.timestep_variance_scores[-10:]
            ) / min(10, len(self.timestep_variance_scores))

        if self.ptss_probabilities:
            stats["fvdm/avg_ptss_probability"] = sum(self.ptss_probabilities) / len(
                self.ptss_probabilities
            )
            stats["fvdm/recent_ptss_probability"] = sum(
                self.ptss_probabilities[-10:]
            ) / min(10, len(self.ptss_probabilities))
    ...
    def get_recent_stats(self, window: int = 100) -> Dict[str, float]:
        ...
        if self.temporal_consistency_losses:
            ...
            recent_stats["fvdm/recent_temporal_consistency"] = sum(recent_losses) / len(
                recent_losses
            )

        if self.frame_diversity_scores:
            recent_diversity = self.frame_diversity_scores[-window:]
            recent_stats["fvdm/recent_frame_diversity"] = sum(recent_diversity) / len(
                recent_diversity
            )

        if self.timestep_variance_scores:
            recent_variance = self.timestep_variance_scores[-window:]
            recent_stats["fvdm/recent_timestep_variance"] = sum(recent_variance) / len(
                recent_variance
            )

        # Recent async ratio
        recent_flags = list(self.recent_async_flags)[-window:]
        recent_async = sum(1 for flag in recent_flags if flag)
...
def compute_fvdm_loss_components(
    ...
    total_loss = temporal_weight * temporal_loss - diversity_weight * diversity_score
