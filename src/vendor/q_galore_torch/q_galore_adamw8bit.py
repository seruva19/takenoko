...
class AdamW8bit(Optimizer2State):
    ...
    def update_step(self, group, p, gindex, pindex, flag_use_float_grad=False):
        ...
        if grad is not None and p.device != grad.device:
            p.data = p.data.to(grad.device)
            if hasattr(p, "saved_data") and isinstance(p.saved_data, torch.Tensor):
                p.saved_data = p.saved_data.to(grad.device)
        ...
        if state["state1"].dtype == torch.float:
            if state["state1"].device != grad.device:
                state["state1"] = state["state1"].to(grad.device)
            if state["state2"] is not None and state["state2"].device != grad.device:
                state["state2"] = state["state2"].to(grad.device)
            ...
            if state["state2"] is not None and state["state2"].device != grad.device:
                state["state2"] = state["state2"].to(grad.device)
            if state["qmap1"].device != grad.device:
                state["qmap1"] = state["qmap1"].to(grad.device)
            if state["qmap2"] is not None and state["qmap2"].device != grad.device:
                state["qmap2"] = state["qmap2"].to(grad.device)
            if state["max1"].device != grad.device:
                state["max1"] = state["max1"].to(grad.device)
            if state["max2"] is not None and state["max2"].device != grad.device:
                state["max2"] = state["max2"].to(grad.device)
            if state["new_max1"].device != grad.device:
                state["new_max1"] = state["new_max1"].to(grad.device)
            if state["new_max2"] is not None and state["new_max2"].device != grad.device:
                state["new_max2"] = state["new_max2"].to(grad.device)
            ...
            if state["qmap2"] is not None and state["qmap2"].device != grad.device:
                state["qmap2"] = state["qmap2"].to(grad.device)
            if state["absmax1"].device != grad.device:
                state["absmax1"] = state["absmax1"].to(grad.device)
            if state["absmax2"] is not None and state["absmax2"].device != grad.device:
                state["absmax2"] = state["absmax2"].to(grad.device)
            blockwise_params = inspect.signature(
                F.optimizer_update_8bit_blockwise
            ).parameters
            if "beta3" in blockwise_params or len(blockwise_params) >= 18:
                F.optimizer_update_8bit_blockwise(
                    self.optimizer_name,
                    grad,
                    p,
