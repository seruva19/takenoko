...
class W8Linear(torch.autograd.Function):
    ...
    def forward(ctx, x, weight, bias):
        ...
        def forward_w_float_weight(weight, x, bias):
            if weight.zeros.device != weight.device:
                weight.zeros = weight.zeros.to(weight.device)
            if weight.scales.device != weight.device:
                weight.scales = weight.scales.to(weight.device)
    ...
    def backward(ctx, grad_output):
        ...
        def backward_w_float_weight(weight, grad_output):
            ...
            if weight.scales.device != weight.device:
                weight.scales = weight.scales.to(weight.device)
            float_weight = weight.to(grad_output.dtype).reshape(-1, weight.group_size)
            zeros = weight.zeros.to(weight.device, dtype=float_weight.dtype)
            scales = weight.scales.to(weight.device, dtype=float_weight.dtype)
        ...
        grad_matrix = grad_output.reshape(-1, out_features).t()
        x_matrix = x.reshape(-1, in_features)
        if grad_matrix.dtype != x_matrix.dtype:
            x_matrix = x_matrix.to(grad_matrix.dtype)
        if weight.float_grad is not None:
            weight.float_grad += grad_matrix @ x_matrix
        else:
            weight.float_grad = grad_matrix @ x_matrix
...
class QGaLoreLinear(nn.Module):
    def __init__(
        ...
        int8_weight, scales, zeros = _quantize_tensor_int8(
            weight.data, q_group_size=group_size
        )
        torch.cuda.empty_cache()

        self.weight = Parameter(int8_weight, requires_grad=False).to(
            device
        )  # Only Tensors of floating point and complex dtype can require gradients, using float_gradient to store the gradient
        ...
        self.bias = (
            Parameter(bias, requires_grad=True).to(device) if bias is not None else None
        )
...
def prepare_model_for_int8_training(model, args, target_module):
    ...
    for name, module in reversed(model._modules.items()):
        ...
        if len(list(module.children())) > 0:
            model._modules[name] = prepare_model_for_int8_training(
