...
class GradientProjector:
    ...
    def _align_ortho_matrix(self, reference_tensor):
        """Ensure cached projection matrices stay on the same device as incoming tensors."""
        if self.ortho_matrix is None:
            return
        device = reference_tensor.device
        if isinstance(self.ortho_matrix, list):
            self.ortho_matrix = [matrix.to(device=device) for matrix in self.ortho_matrix]
        else:
    ...
    def project(self, full_rank_grad, iter):
        ...
        if self.proj_type == "std":
            if full_rank_grad.shape[0] >= full_rank_grad.shape[1]:
                ...
                self._align_ortho_matrix(full_rank_grad)
    ...
    def project_back(self, low_rank_grad):
        ...
        self._align_ortho_matrix(low_rank_grad)
